...
export const votingRouter = createTRPCRouter({
  submitVote: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.optionId) {
        await ctx.prisma.postOption.update({
      ...
      if (voter && !isSelfVote) {
        const newScore = computeOpinionScoreForVote(
          voter.opinionScore,
          voter.totalVotes,
        );
      ...
      if (updatedPost) {
        const optionCounts = updatedPost.options.map((o) => o.voteCount);
        ...
        return {
          ...
          trendingScore: trending,
          viralityScore: virality,
          options: updatedPost.options.map((o) => ({
            id: o.id,
